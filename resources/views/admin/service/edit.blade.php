...
<div class="content-wrapper">
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6"><h1>Edit Service</h1></div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{url('admin/dashboard')}}">Home</a></li>
            <li class="breadcrumb-item"><a href="{{url('admin/service/list')}}">Services</a></li>
  ...
  <section class="content">
    <div class="container-fluid">
      ...
      <form method="post" action="{{url('/admin/service/update')}}/{{$edit_article_details->id}}" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="id" value="{{$edit_article_details->id}}">
        <div class="row">
          <div class="col-lg-8">
            <div class="card card-primary">
              <div class="card-header"><h3 class="card-title mb-0">Service Content</h3></div>
              <div class="card-body">
                <div class="form-group">
                  <label>Title <span class="required-star">*</span></label>
                  <input type="text" name="title" id="title" value="{{$edit_article_details->title}}" class="form-control" required>
                </div>
                <div class="form-group">
                  <label>URL / Slug <span class="required-star">*</span></label>
                  <input type="text" name="slug" id="slug" value="{{$edit_article_details->slug}}" class="form-control" required>
                </div>
                <div class="form-group">
                  <label>Content <span class="required-star">*</span></label>
                  <textarea class="ckeditor form-control" name="content" id="content">{{$edit_article_details->content}}</textarea>
                </div>
              </div>
            </div>
          </div>
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="category" id="category">
                    @foreach(['skin-treatment'=>'Skin Treatment','cosmetic-treatment'=>'Cosmetic Treatment','hair-treatment'=>'Hair Treatment','laser-treatment'=>'Laser Treatment','skin-rejuvenation-treatment'=>'Skin Rejuvenation Treatment','make-over'=>'Make Over'] as $val=>$label)
                      <option value="{{$val}}" {{ $edit_article_details->category == $val ? 'selected' : '' }}>{{$label}}</option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group">
                  ...
                  @if ($edit_article_details->featured_image)
                    <div class="mb-2"><img src="{{ asset($edit_article_details->featured_image) }}" alt="Featured" class="img-fluid rounded" style="max-height:160px;"></div>
                  @endif
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div class="form-group">
                  ...
                  <input type="text" name="seo_title" id="seo_title" value="{{$edit_article_details->seo_title}}" class="form-control">
                </div>
                <div class="form-group">
                  <label>SEO Description</label>
                  <textarea class="form-control" name="seo_description" id="seo_description" rows="3">{{$edit_article_details->seo_description}}</textarea>
                </div>
              </div>
            </div>
          </div>

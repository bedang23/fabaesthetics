...
<div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6"><h1>Blogs</h1></div>
          <div class="col-sm-6">
      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-header d-flex align-items-center justify-content-between">
            <h3 class="card-title mb-0"><i class="fas fa-newspaper mr-2"></i>All Blogs
              <span class="badge badge-secondary ml-2">{{ count($record_details) }}</span>
            </h3>
            <a href="{{url('admin/blog/create')}}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> New Blog</a>
          </div>
                  ...
                  @forelse ($record_details as $value)
                    <tr>
                        ...
                        <td class="text-center fab-actions" style="white-space:nowrap;">
                          <a href="{{ route('blog.show', ['category'=>$value->category,'slug'=>$value->slug]) }}" target="_blank" class="btn btn-sm btn-outline-secondary" title="View"><i class="fas fa-eye"></i></a>
                          <a href="{{url('/admin/blog/edit')}}/{{$value->id}}" class="btn btn-sm btn-primary" title="Edit"><i class="fas fa-edit"></i></a>
                          <a href="javascript:void(0);" onclick="deleteRow({{$value->id}})" class="btn btn-sm btn-danger" title="Delete"><i class="fas fa-trash"></i></a>
                        </td>
                    </tr>
                  @empty
                    <tr><td colspan="8" class="text-center fab-cell-muted py-4">No blogs yet. Click “New Blog” to add one.</td></tr>
                  @endforelse
                </tbody>
            </table>
          </div>
...
<script type="text/javascript">
    function deleteRow(id) {
      if (confirm("Are you sure you want to delete this blog? This cannot be undone.")) {
        window.location.href = '{{url("/admin/blog/delete")}}/' + id;
      }
    }
</script>
@endsection
